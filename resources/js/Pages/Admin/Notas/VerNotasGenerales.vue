<template>
    <AppLayout title="Evaluación General">
        <template #header>
        ...
        <div class="bg-blue-100 m-10 flex flex-col items-center rounded-md shadow-md p-2">
            <div class="w-full">
                <div>
                    ...
                    <p>
                        <span class="font-semibold">Tipo evaluacion: </span>
                ...
                <form class="form-control w-full" @submit.prevent="form.post(route('admin.evaluar-generales.store', { tipo:tipo.id, asignatura:asignatura.id, periodo:periodo.id, grupo:grupo.id, actividad:actividad ? actividad.id : null }))">
...
const title = ref('Evaluación ' + props.grupo.nombre);
...
function cancelar()
{
    Inertia.visit(route('admin.panel-evaluaciones.show', { periodo:props.periodo.id, grupo:props.grupo.id, asignatura:props.asignatura.id }));
}
