    ...
    <AppLayout title="Escala de Valoración">
        ...
        <div class="bg-blue-100 m-10 flex flex-col items-center rounded-md shadow-md p-2">
            <div class="card w-xs bg-base-100 shadow-xl">
                <div class="card-body">
                    <h2 class="card-title">Editar escala de valoración</h2>        
                    
                    <div class="flex flex-col items-center">
                        <div>                
                            <!--Se usa método POST en la actualización, ya que es necesario en Inertia para poder subir la imagen-->
                            <form class="form-control w-full max-w-xs text-sm" @submit.prevent="form.post(route('admin.escala-valoracion.update', form.id))">
...
const form = useForm({
    _method: 'PUT',
    id: props.escala.id,
    grado_id: props.escala.grado_id,
    nombre: props.escala.nombre,
    abreviatura: props.escala.abreviatura,    
    rango_inicial: props.escala.rango_inicial,
    rango_final: props.escala.rango_final,
    imagen: null,
});
